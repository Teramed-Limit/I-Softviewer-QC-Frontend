...
export class DicomDirectoryCreator {
    factoryMapper: Record<string, (dataSet, fileList?) => DicomDirectoryRecord> = {
        ...
        [DirectoryRecordNodeType.IMAGE]: (dataSet, fileList) => new ImageDirectoryRecordNode(dataSet, fileList),
        [DirectoryRecordNodeType.SRDOCUMENT]: (dataSet, fileList) => new SRDocDirectoryRecordNode(dataSet, fileList),
    };

    createRecordInstance(explicitDataSet: ExplicitDataSet, fileList: FileList): DicomDirectoryRecord | null {
        // directoryRecordType
        ...
        return creator(dataset, fileList);
    }
